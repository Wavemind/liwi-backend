...
class Diagnostic < ApplicationRecord
  ...
  def questions_json
    generate_questions_order.as_json(include: [conditions: { include: [first_conditionable: { methods: :get_node }, second_conditionable: { methods: :get_node }] }, node: { include: [:answers] }])
  end

  # @return [Json]
  # Return final diagnostics in json format
  def final_diagnostics_json
    components.final_diagnostics.as_json(include: [ :node, conditions: { include: [first_conditionable: { methods: :get_node }, second_conditionable: { methods: :get_node }]}])
  end

  # @return [Json]
  # Return treatments and managements in json format
  def health_cares_json
    components.treatments.as_json(include: [ :node, conditions: { include: [first_conditionable: { methods: :get_node }, second_conditionable: { methods: :get_node }]}]) + components.managements.as_json(include: [ :node, conditions: { include: [first_conditionable: { methods: :get_node }, second_conditionable: { methods: :get_node }]}])
  end
