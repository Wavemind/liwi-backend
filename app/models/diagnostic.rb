...
class Diagnostic < ApplicationRecord
  ...
  def available_nodes_json
    (version.algorithm.nodes.where(stage: nil).or(version.algorithm.nodes.where.not('stage = ? AND type != ? ', Question.stages[:triage], 'Questions::ChiefComplaint')).where.not(id: components.not_health_care_conditions.select(:node_id)).where.not('type LIKE ?', 'HealthCares::%').includes(:answers) + final_diagnostics.where.not(id: components.select(:node_id))).as_json(methods: [:category_name, :node_type, :get_answers, :type])
  end
