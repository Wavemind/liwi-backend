    ...
    components.includes(:node, :children, :conditions).each do |instance|
      if instance.node.is_a? FinalDiagnostic
        ...
        instance.conditions.each do |cond|
          age_missing = false if cond.first_conditionable.is_a?(Answer) && cond.first_conditionable.node.formula.present? && cond.first_conditionable.node.formula.include?('BD1')
        end
