...
class Formulation < ApplicationRecord
  ...
  validates_presence_of :minimal_dose_per_kg, :maximal_dose_per_kg, :maximal_dose, :dose_form, if: Proc.new { %w(tablet capsule syrup suspension solution powder_for_injection).include?(self.medication_form) && !( self.by_age == '1' || self.by_age == true)}
  ...
  validates_presence_of :unique_dose, unless: Proc.new { %w(tablet capsule syrup suspension solution powder_for_injection).include?(self.medication_form) || ( self.by_age == '1' || self.by_age == true) }

  after_validation :validate_dosage_logic, if: Proc.new { %w(tablet capsule syrup suspension solution powder_for_injection).include?(self.medication_form) && !( self.by_age == '1' || self.by_age == true)}
