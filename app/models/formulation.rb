...
class Formulation < ApplicationRecord
  ...
  # Validate presence of fields needed to calculate dosage for tablets, capsules, syrup, solution and suspension (unless the condition is based on the age)
  validates_presence_of :minimal_dose_per_kg, :maximal_dose_per_kg, :maximal_dose, :dose_form, if: Proc.new { %w(tablet capsule syrup suspension solution).include?(self.medication_form) && !( self.by_age == '1' || self.by_age == true)}
  # Validate presence of liquid concentration needed to calculate dosage for syrup, solution and suspension (unless the condition is based on the age)
  validates_presence_of :liquid_concentration, if: Proc.new { %w(syrup suspension solution).include?(self.medication_form) && !( self.by_age == '1' || self.by_age == true)}
  ...
  validates_presence_of :unique_dose, unless: Proc.new { %w(tablet capsule syrup suspension solution).include?(self.medication_form) || ( self.by_age == '1' || self.by_age == true) }

  after_validation :validate_dosage_logic, if: Proc.new { %w(tablet capsule syrup suspension solution).include?(self.medication_form) && !( self.by_age == '1' || self.by_age == true)}
