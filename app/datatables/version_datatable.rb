class VersionDatatable < AjaxDatatablesRails::ActiveRecord
  ...
  def data
    records.map do |record|
      actions = link_to(I18n.t('show'), algorithm_version_url(record.algorithm, record), class: 'btn btn-outline-primary') + " " + link_to(I18n.t('edit'), edit_algorithm_version_url(record.algorithm, record), class: 'btn btn-outline-info') + " "
