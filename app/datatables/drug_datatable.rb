class DrugDatatable < AjaxDatatablesRails::ActiveRecord
  ...
  def data
    records.map do |record|
      actions = link_to(I18n.t('edit'), edit_algorithm_drug_url(params[:id], record), class: 'btn btn-outline-info') + " " + link_to(I18n.t('delete'), algorithm_drug_url(record.algorithm, record), class: "btn btn-outline-danger #{record.dependencies? ? 'disabled' : ''}", method: :delete, data: { confirm: I18n.t('confirmation') })
      {
        id: record.id,
