    ...
    Question.steps.each do |step_name, step_index|
      ...
      if %w(medical_history_step physical_exam_step).include?(step_name)
        Question.systems.each do |system_name, system_index|
          system_hash = {}
          system_hash[system_name] = full_order.select{|i| i['title'] == I18n.t("questions.systems.#{step_name}")}[0]['children'].select{|i| i['title'] == I18n.t("questions.steps.#{system_name}")}[0]['children'].map{|node| node['id']}
