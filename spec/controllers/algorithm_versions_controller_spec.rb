...
RSpec.describe AlgorithmVersionsController, type: :controller do
  before(:each) do
    ...
    @version = AlgorithmVersion.create!(version: 'final', json: '{}', algorithm: algorithm, user: user)
    @archived_version = AlgorithmVersion.create!(version: 'obsolete', json: '{}', algorithm: algorithm, user: user, archived: true)
  end

  it 'archives correctly' do
    put :archive, params: {id: @version.id}
    @version.reload
    expect(@version.archived).to equal(true)
  end

  it 'unarchives correctly' do
    put :unarchive, params: {id: @archived_version.id}
