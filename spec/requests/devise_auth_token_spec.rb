...
describe 'Whether access is ocurring properly', type: :request do
  before(:each) do
    ...
    epoc_first = AlgorithmVersion.create!(name: 'first_trial', algorithm: epoct, user: @current_user)
  ...
  context 'context: general authentication via API, ' do
    ...
    it 'first get a token, then try to access with mac_address to a restricted page' do
      login
      auth_params = get_auth_params_from_login_response_headers(response)
      get api_v1_algorithm_versions_url, headers: auth_params, params: { mac_address: '64:DB:43:D5:31:5C' }
